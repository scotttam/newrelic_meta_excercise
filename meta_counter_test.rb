...
class MetaCounterTest < Test::Unit::TestCase
  ...
  def test_integer_addition
    puts " SCOTT - I have not figured this one out yet."
  ...
  # #ADDED BY SCOTT - handle prepend in Ruby 2.0
  if RUBY_VERSION == '2.0.0'
    def test_method_prepended_via_module
      output = execute_program('B#b', <<-RB)
      module A; def b; end; end
      class B; prepend A; end
      10.times{ B.new.b }
      RB
      assert_sig_and_count(output, 'B#b', 10)
    end
  
    def test_method_prepended_via_module_and_counted_through_module
      output = execute_program('A#b', <<-RB)
      module A; def b; end; end
      class B; prepend A; end
      10.times{ B.new.b }
    ...
    def test_method_prepended_then_extended_into_object
      output = execute_program('B#b', <<-RB)
      module A; def b; end; end
      class B; prepend A; end
      b = B.new
    ...
    def test_method_prepended_then_extended_into_object_and_counted_through_module
      output = execute_program('A#b', <<-RB)
      module A; def b; end; end
      class B; prepend A; end
      b = B.new
